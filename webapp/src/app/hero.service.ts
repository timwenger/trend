...
export class HeroService {
  ...
  getHeroes(): Observable<Hero[]> {
    ...
    .pipe(
      tap(_ => this.logMsg('fetched heroes')),
...
addHero(hero: Hero): Observable<Hero> {
  ...
  .pipe(
    tap((newHero: Hero) => this.logMsg(`added hero w/ id=${newHero.id}`)),
...
searchHeroes(term: string): Observable<Hero[]> {
  ...
  .pipe(
    tap(x => x.length ?
       this.logMsg(`found heroes matching "${term}"`) :
       this.logMsg(`no heroes matching "${term}"`)),
